...
## ALGORITHM:
Create sockets at both client and server sides for communication.

Server binds to an IP and port, then listens for client connection requests.

Client connects to the server using the server’s IP address and port number.

Server accepts the connection and begins receiving messages from the client.

Both client and server exchange messages continuously using send() and recv() until the user sends "quit".

Close all connections by terminating the client and server sockets.
